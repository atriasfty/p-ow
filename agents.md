...
**External uptime monitoring lives entirely outside this repo.** AUAS (Atria Uptime Alert System — Uptime Kuma → Discord + PagerDuty) checks the bot via a Cloudflare Tunnel public hostname, `https://botstatus.ciankelly.xyz/health` → `127.0.0.1:41734` (bot prod's `BOT_HEALTH_PORT`), configured only in the Cloudflare Zero Trust dashboard and Uptime Kuma. Nothing in this repo grep's for it. If `BOT_HEALTH_PORT` ever changes, this tunnel's backend target must be manually updated to match, or AUAS will report the bot down while PM2/Grafana show it healthy — this exact false alarm happened 2026-08-13 when the tunnel was left on the old `41732`. If AUAS/Grafana disagree about bot health, suspect this tunnel before assuming a code regression.
...
1. **App-level metrics/tracing code** — lives in the normal repo, deploys with everything else.
   ...
   - `dashboard/src/app/api/internal/infra-alert/route.ts` — receives Alertmanager's webhook for infra-only alerts (host mem/disk, PM2 process down), internal-secret gated like `/api/internal/sync`, and forwards through the existing `sendAlert()` in `alerting.ts` onto a **separate** `INFRA_ALERT_DISCORD_WEBHOOK_URL` channel from app alerts (`ALERT_DISCORD_WEBHOOK_URL`) — pass `sendAlert({..., channel: "infra"})`. One cooldown/dedup system, two Discord destinations. All `sendAlert()` messages are prefixed with `@everyone` in the message content so they actually page someone. `sendAlert()` increments `pow_alert_send_failures_total{channel}` when the Discord webhook `fetch` itself fails/non-2xxs — this is the alerting pipeline's own dead-man's switch, since a broken webhook would otherwise fail completely silently.
   - `dashboard/src/lib/http-metrics.ts` — `withHttpMetrics(route, handler)`, records `pow_http_request_duration_seconds{route,method,status}`. Wraps `public/v1/*` (paid API), `internal/sync`, `internal/infra-alert`, `admin/projector-stats`, `maintenance-check` — **partial coverage, not every route**. `route` is always a hardcoded label, never taken from the URL, to keep cardinality bounded. Wrap new routes the same way as they're added if you want them covered.
   - `pow_prc_key_invalid_servers` gauge and `pow_automation_executions_total{type,outcome}` / `pow_automation_duration_seconds{type}` (automation-engine.ts) round out app-level metrics beyond the original sync/API/DB set.
