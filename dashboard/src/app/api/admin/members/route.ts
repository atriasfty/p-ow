...
export async function GET(req: Request) {
    ...
    try {
        ...
        // ⚡ Bolt: Fetch the single most-recent shift per user using distinct and orderBy to avoid N+1 and large OR arrays
        const latestShifts = await prisma.shift.findMany({
            where: { serverId, userId: { in: userIds } },
            distinct: ['userId'],
            orderBy: [
                { userId: 'asc' }, // Required first element for distinct with postgres
                { startTime: 'desc' }
            ],
