...
export async function GET(req: Request) {
    ...
    const remoteAddr = req.headers.get("x-real-ip") || req.headers.get("remote-addr")

    // Never trust the first XFF entry for auth/rate-limiting
    const lastXff = forwardedFor ? forwardedFor.split(",").at(-1)?.trim() : undefined
    const ip = cfIp || lastXff || remoteAddr || "unknown"
