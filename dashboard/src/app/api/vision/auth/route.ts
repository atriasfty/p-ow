...
export async function GET(req: Request) {
    try {
        ...
        if (!session?.user) {
            return NextResponse.json({ error: "Not authenticated" }, { status: 401, headers: getVisionCorsHeaders(req.headers.get("origin")) })
        }
        ...
        if (!hasAccess) {
            return NextResponse.json({ error: "Vision access requires a Pro User subscription or membership in a Max tier server." }, { status: 403, headers: getVisionCorsHeaders(req.headers.get("origin")) })
        }
        ...
        return NextResponse.json({ error: "Failed to generate token" }, { status: 500, headers: getVisionCorsHeaders(req.headers.get("origin")) })
    }
}
...
export async function POST(req: Request) {
    try {
        ...
        if (!token) {
            return NextResponse.json({ valid: false, error: "No token provided" }, { status: 400, headers: getVisionCorsHeaders(req.headers.get("origin")) })
        }
        ...
        return NextResponse.json({ valid: false, error: "Invalid token" }, { status: 401, headers: getVisionCorsHeaders(req.headers.get("origin")) })
    }
}
